    ...
    try {
        const url = await axios.get(`https://www.instagram.com/${widgetSetting.id}`).then(fetchData => {
            ...
            photos.forEach(photo => {
                photo.caption ? photo.caption = photo.caption.node.text : photo.caption = " ", photo.likesCount > 0 ? photo.likesCount = "<span>&#x2764;</span> " + nFormat(photo.likesCount) : photo.likesCount = " ", photo.commentCount > 0 ? photo.commentCount = "<span>&#x1F4AC;</span> " + nFormat(photo.commentCount) : photo.commentCount = " ";
            ...
            }), footer.classList.add("widget-footer"), footer.innerHTML = `<a href="https://instagram.com/${widgetSetting.id}" class="widget-button" target="_blank" style="background-color:${widgetSetting.color}">View on Instagram</a> `, container.appendChild(footer)
        })
    } catch (e) {
